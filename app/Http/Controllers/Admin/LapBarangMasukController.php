    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            if ($request->tglawal == '') {
                ...
                $data = BarangmasukModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangmasuk.barang_kode')->leftJoin('tbl_user', 'tbl_user.user_id', '=', 'tbl_barangmasuk.user_id')->whereBetween('bm_tanggal', [$request->tglawal, $request->tglakhir])->orderBy('bm_id', 'DESC')->get();
            }
            return DataTables::of($data)
                ...
                ->addColumn('user', function ($row) {
                    $user_id = $row->user_id == '' ? '-' : $row->user_nama;
